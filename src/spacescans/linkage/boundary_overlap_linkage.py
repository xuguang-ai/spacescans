# common_v2/linkage/boundary_overlap_linkage.py
...
from spacescans.geometry.overlap import compute_overlap_weights
...
@register_pattern("boundary_overlap")
def run_boundary_overlap(config: DatasetConfig, engine: AggregationEngine) -> Path:
    ...
    weights = compute_overlap_weights(
        ...
        raster_cache_dir=Path(config.buffer.raster_cache_dir) if config.buffer.raster_cache_dir else None,
    )
